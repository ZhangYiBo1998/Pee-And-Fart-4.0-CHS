...
Idle property paf_ZAZAPC203 auto
Idle property paf_ZAZAPC204 auto
Idle property paf_ZAZAPC211 auto
Idle property paf_ZAZAPC205 auto
...
Idle property paf_ZAZAPC207 auto
Idle property paf_ZAZAPC208 auto

; gender invariant animations
Idle property paf_ZAZAPC201 auto
Idle property paf_ZAZAPC202 auto
Idle property paf_ZAZAPC209 auto
Idle property paf_ZAZAPC210 auto

; idle reset (Credit to ZAZ and XAZ)
Idle property PAF_IdleReset auto
...
Sound property PAF_SoundMoanFemale auto; Credit to ZAZ
Sound property PAF_SoundMoanMale auto; Credit to ZAZ
...
Static property PAF_ZaZAPEPeePuddleSmall auto
Static property PAF_ZaZAPEPeePuddleLarge auto
	...
	if ((PeeState > 3 || PoopState > 3) && !isAnimating && ReadyToAnimate(PlayerREF))
		...
		if (last_animation_check == 1)			
			...
			if (!PAF_DDQuest.HasArmbinder(PlayerREF) && PAF_MCMQuest.PAF_PressureAnimation && !isAnimating)
				...
				PlayerREF.PlayIdle(PAF_IdleReset)
				isAnimating = false
		...
		elseif (result == 3)
			...
			elseif (result_more == 2)
				...
				if (ref != none)
					...
					if (result_toilet == 0)
						PAF_ToiletQuest.PlaceToilet(ref, 1)
					elseif (result_toilet == 1)
						PAF_ToiletQuest.PlaceToilet(ref, 2)
					elseif (result_toilet == 2)
						PAF_ToiletQuest.PlaceToilet(ref, 3)
					endif
				endif
...
function CalculateStates()
	...
	int handle = ModEvent.Create("PAF_needsEvent")
	if (handle)
		ModEvent.PushInt(handle, PoopState)
		ModEvent.PushInt(handle, PeeState)
...
function SetLastEaten(float amount)
	last_eaten = GetCurrentGameTime()		
	last_poop = last_poop - (amount * 0.005 * ((PAF_MCMQuest.PAF_PoopRate as float) / 24.0))
endFunction
...
int function PlayPeeIdleStart(Actor a_actor, int style, bool female)
	if (female)
		...
		elseif (style == 4)
			a_actor.PlayIdle(paf_ZAZAPC201)
		elseif (style == 5)
			a_actor.PlayIdle(paf_ZAZAPC202)
		elseif (style == 6)
			a_actor.PlayIdle(paf_ZAZAPC203)
		elseif (style == 7)
			a_actor.PlayIdle(paf_ZAZAPC204)
		elseif (style == 8)
			a_actor.PlayIdle(paf_ZAZAPC209)
		elseif (style == 9)
			a_actor.PlayIdle(PAF_Pose0)
		elseif (style == 10)
			a_actor.PlayIdle(paf_ZAZAPC205)
		elseif (style == 11)
		...
		elseif (style == 3)
			a_actor.PlayIdle(PAF_Pee04_Start)
		elseif (style == 4)
			a_actor.PlayIdle(paf_ZAZAPC207)
		elseif (style == 5)
			a_actor.PlayIdle(paf_ZAZAPC208)
		elseif (style == 6)
			a_actor.PlayIdle(paf_ZAZAPC209)
		elseif (style == 7)
			a_actor.PlayIdle(paf_ZAZAPC210)
		elseif (style == 8)
			a_actor.PlayIdle(PAF_Pose0)
		elseif (style == 9)
			a_actor.PlayIdle(paf_ZAZAPC211)
		elseif (style == 10)
			a_actor.PlayIdle(PAF_Pose1)
		elseif (style == 11)
			a_actor.PlayIdle(PAF_Bendover01)
...
function PlayPeeIdleStop(Actor a_actor, int style, bool female)
	if (female)
		...
		elseif (style == 4)
			a_actor.PlayIdle(PAF_IdleReset)
		elseif (style == 5)
			a_actor.PlayIdle(PAF_IdleReset)
		elseif (style == 6)
			a_actor.PlayIdle(PAF_IdleReset)
...
function ScaleBelly(Actor a_actor)
	if (PAF_MCMQuest.PAF_Scaling && a_actor.GetActorBase().GetSex() == 1)		
		NiOverride.SetBodyMorph(PlayerREF, "PregnancyBelly", "PAF_Belly_Scale", MinFloat(MinInt(PoopState, 4) * 0.25, 1.0) * PAF_MCMQuest.PAF_ScalingFactor)		
...
function PlayPoopSound(Actor a_actor)
	if (PAF_MCMQuest.PAF_PlayMoaningSounds)
		if (a_actor.GetActorBase().GetSex() == 0)
			PAF_SoundMoanMale.Play(a_actor)
		else
			PAF_SoundMoanFemale.Play(a_actor)
		endif
	endif
	Wait(0.3)
...
function EquipTinke(Actor a_actor, bool parabolic = false, bool leak = false)
	...
	elseif ((a_actor.GetBaseObject() as ActorBase).GetSex() == 0)
		...
		if (SexLab.GetGender(a_actor) == 0)
			...
			if (parabolic)
				a_actor.EquipItem(PAF_ZaZAPEF2Pee02_M, true, true)
			else
				a_actor.EquipItem(PAF_ZaZAPEF2Pee01_M, true, true)
			endif
		endif
...
function UnequipTinke(Actor a_actor, bool parabolic = false, bool leak = false)	
	a_actor.RemoveItem(PAF_ZaZAPEF2Pee02, 1, true)
	a_actor.RemoveItem(PAF_ZaZAPEF2Pee02_M, 1, true)
...
int function PeeInMouth()
	return StartSexlabAnimation(playerREF, VictimActor, SexlabDoPee(), false);
endfunction

int function PeeInMouthPlayer()
	return StartSexlabAnimation(VictimActor, playerREF, SexlabDoPee(), false);
endFunction

int function PoopInMouth()
	return StartSexlabAnimation(playerREF, VictimActor, false, SexlabDoPoop());
endfunction

int function PoopInMouthPlayer()
	return StartSexlabAnimation(VictimActor, playerREF, false, SexlabDoPoop());
endFunction

int function Masturbate()
	return StartSexlabAnimation(playerREF, none, SexlabDoPee(), SexlabDoPoop());
endFunction

bool function SexlabDoPee()
	bool slpee = true	
