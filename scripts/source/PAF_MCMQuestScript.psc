...
Event OnOptionSelect(int a_option)
	...
	elseif (a_option == _toggle_Scaling)
		...
		if (!PAF_Scaling)
			NiOverride.SetBodyMorph(PlayerREF, "PregnancyBelly", "PAF_Belly_Scale", 1)
			NiOverride.ClearBodyMorphKeys(PlayerREF, "PAF_Belly_Scale")
...
event OnOptionHighlight(int a_option)
	...
	elseIf (a_option == _keymap_Info)
        ...
        SetInfoText("按此键显示自身需求及已追踪角色的状态信息（需瞄准目标）。\n默认键位：N")
	...
	elseIf (a_option == _value_ScalingFactor)
        ...
        SetInfoText("腹部调整的缩放系数。该值会乘以你的直肠状态值+1[1-4]！\n默认值：0.75")
